...
class ReconcileOktoWithdrawalJob implements ShouldQueue
{
    use Dispatchable;
    ...
    public function handle(): void
    {
        ...
        if ($withdrawal === null
            ...
            || $withdrawal->payout_provider !== 'okto') {
            return;
        }

        $tx = trim((string) $withdrawal->payout_external_id);
        if ($tx === '') {
            $tx = (string) $withdrawal->id;
        }
        ...
        try {
            ...
            Log::warning('ReconcileOktoWithdrawalJob: falha na consulta', [
        ...
        if (in_array($apiStatus, ['cancelled', 'failed'], true)) {
            MerchantWithdrawalService::markFailed(
                $withdrawal->fresh(),
                'Payout Okto falhou (reconciliação): '.(string) $apiStatus
            );
    ...
    private function maybeReleaseForRetry(): void
    {
        ...
        if ($this->attempts() >= 36) {
            ...
            if ($withdrawal !== null && in_array($withdrawal->status, ['pending', 'processing'], true)) {
                MerchantWithdrawalService::markFailed(
                    $withdrawal->fresh(),
                    'Payout Okto não confirmado após esgotar tentativas de reconciliação.'
                );
            }
